...
class UserModel extends MasterModel {
  ...
  async findByIdSafe(id, pool) {
    const query = `SELECT id, name, email, phone, profile_photo, role, sponsor_code, site_id, sponsor_id, parent_id, is_active, created_at, updated_at FROM ${this.tableName} WHERE id = $1`;
